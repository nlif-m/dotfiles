...
nvim: 
	...
	ln -s $(P-dotfiles)/vim-neovim/init.vim $(P-nvim)/init.vim $(DEV_NULL);

zshrc:
	mkdir -p $(P-zsh) $(DEV_NULL); 
	rm -f $(P-zsh)/.zshrc $(DEV_NULL); 
	ln -s $(P-dotfiles)/zsh/.zshrc $(P-zsh)/.zshrc $(DEV_NULL);

xinitrc:
	mkdir -p $(P-x11) $(DEV_NULL); 
	rm -f $(P-x11)/.xinitrc $(DEV_NULL);
	ln -s $(P-dotfiles)/xinitrc/.xinitrc $(P-x11)/.xinitrc $(DEV_NULL);
...
xmonad: xmobar xmonadMarkdown
	...
	ln -s $(P-dotfiles)/xmonad-xmobars/xmonad.hs $(P-xmonad)/xmonad.hs 

xmobar:
	mkdir -p $(P-xmobar) $(DEV_NULL)
	rm -f $(P-xmobar)/xmobarrc0 $(P-xmobar)/xmobarrc1 $(DEV_NULL)
	ln -s $(P-dotfiles)/xmonad-xmobars/xmobarrc0 $(P-xmobar)/xmobarrc0
	ln -s $(P-dotfiles)/xmonad-xmobars/xmobarrc1 $(P-xmobar)/xmobarrc1

xmonadMarkdown:
	mkdir -p $(P-xmonad) $(DEV_NULL)
	rm -f $(P-xmonad)/xmonad.md $(DEV_NULL)
	ln -s $(P-dotfiles)/xmonad-xmobars/xmonad.md $(P-xmonad)/xmonad.md
